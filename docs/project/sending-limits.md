...
- **自动撤回需要 message_id**：`auto_revoke_scope` 命中时，OneBot/NapCat 类平台的 direct、HTML fallback、stream、file:// 直通、forward 和 R18 Docx 会优先走 raw OneBot action 以提取 `message_id`。拿不到 id、平台不支持 `delete_msg` 或删除失败时只记录 warning，不触发重复发送或阻断发图。
